...
-- Final 19-table PostgreSQL schema with sample data
...
BEGIN
    INSERT INTO store_inventory (store_id, product_id, quantity_on_hand, reorder_threshold)
...
BEGIN
    SELECT store_id INTO sale_store_id
    FROM sales_transactions
    WHERE sale_id = NEW.sale_id;

    INSERT INTO inventory_adjustments (store_id, product_id, adjustment_type, quantity_delta, notes)
    VALUES (sale_store_id, NEW.product_id, 'sale', -NEW.quantity_sold, 'Automatic inventory reduction from sale');
...
SELECT
    ...
    SUM(st.total_amount - st.refund_amount) AS net_sales
FROM sales_transactions st
JOIN stores s ON s.store_id = st.store_id
WHERE st.sale_status IN ('completed', 'partially_refunded')
...
SELECT
    ...
    AVG(po.received_date - po.expected_date) AS avg_days_late,
